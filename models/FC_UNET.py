...
def calc_out_dim(stride, input_size, filter_size, padding):
    return (stride*(input_size-1) + filter_size - 2*padding)
...
class FC_UNET(nn.Module):
    def __init__(self):
        ...
        self.stride_default = 1
        # Input Tensor Dimensions = 256x256x3
        # Convolution 1
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=5, stride=self.stride_default, padding=2)
        ...
        next_input_size = calc_out_dim(self.stride_default, 16, 5, 2)
        print(next_input_size)
        ...
        next_input_size = calc_out_dim(self.stride_default, 32, 3, 1)
        print(next_input_size)
        ...
        next_input_size = calc_out_dim(self.stride_default, 64, 3, 1)
        print(next_input_size)
